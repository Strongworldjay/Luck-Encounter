...
},
{
  ...
  ritual: false,
  descriptionMd:
    "You summon a **giant centipede, spider, or wasp** (chosen when you cast). It appears in an unoccupied space within range and uses the **Giant Insect** stat block; the chosen form sets certain details. The creature acts after you on your initiative, obeys your verbal commands (no action), and vanishes at **0 HP** or when the spell ends. If not commanded, it **Dodges** and avoids danger.\n\n**Stat Block (summary).** *Large Beast; AC **11 + slot level**; HP **30 + 10/slot above 4**; Speeds **40 ft., climb 40 ft.**; **fly 40 ft.** (wasp). Spider Climb. Multiattack = **half slot level** (round down). **Poison Jab** (melee, your spell attack; reach 10 ft.): **1d6 + 3 + slot level** Piercing + **1d4 Poison**. **Web Bolt** (spider only, ranged 60 ft.): **1d10 + 3 + slot level** Bludgeoning and target’s Speed = 0 until start of its next turn. **Venomous Spew** (centipede only, bonus): Con save (your DC) or **Poisoned** until start of insect’s next turn.",
  scalingMd:
...
},
{
  ...
  ritual: false,
  descriptionMd:
    "You conjure an **invisible, invulnerable** phantom watchdog in an unoccupied space within range; it remains while you are within **300 feet**. When a Small or larger creature comes within **30 ft.** without speaking the **password**, the hound **barks loudly**. It has **Truesight 30 ft.** At the **start of each of your turns**, the hound forces one enemy within **5 ft.** to make a **Dexterity** save or take **4d8 Force**. As a **Magic action** on later turns, move the hound **up to 30 ft.**",
  scalingMd: "",
...
},
{
  ...
  ritual: false,
  descriptionMd:
    "You ward a cube from **5 ft.** to **100 ft.** per side. Choose any of the following properties: **soundproof barrier; opaque fogged barrier; blocks Divination sensors; creatures inside can’t be targeted by Divination; blocks teleportation; blocks planar travel**. Casting on the same spot every day for **365 days** makes the spell **until dispelled**.",
  scalingMd:
...
},
{
  ...
  ritual: false,
  descriptionMd:
    "You call forth an **aberrant spirit** (choose **Beholderkin, Mind Flayer, or Slaad**) in an unoccupied space within range; it uses the **Aberrant Spirit** stat block and vanishes at **0 HP** or when the spell ends. It acts after you on your initiative, obeys your **verbal commands** (no action), or **Dodges** if not commanded.\n\n**Stat Block (summary).** *Medium Aberration; AC **11 + slot level**; HP **40 + 10/slot above 4**; Speed **30 ft.**; **fly 30 ft. (hover)** (Beholderkin); Immunities **Psychic**; Darkvision 60 ft.; languages **Deep Speech**, understands yours. **Regeneration** (Slaad), **Whispering Aura** (Mind Flayer): creatures within 5 ft. make **WIS save** or take **2d6 Psychic**. **Multiattack** = half slot level. **Claw** (Slaad): **1d10 + 3 + slot** Slashing, and target **can’t regain HP** until spirit’s next turn. **Eye Ray** (Beholderkin, 150 ft.): **1d8 + 3 + slot** Psychic. **Psychic Slam** (Mind Flayer): **1d8 + 3 + slot** Psychic.",
  scalingMd:
    "**Using a Higher-Level Spell Slot.** Use the **slot’s level** for the stat block’s level-based values.",
...
},
{
  ...
  ritual: false,
  descriptionMd:
    "You call forth a **construct spirit** (choose **Clay, Metal, or Stone**) in an unoccupied space within range; it uses the **Construct Spirit** stat block and vanishes at **0 HP** or when the spell ends. It acts after you on your initiative, obeys **verbal commands** (no action), or **Dodges** if not commanded.\n\n**Stat Block (summary).** *Medium Construct; AC **13 + slot level**; HP **40 + 15/slot above 4**; Speed **30 ft.**; Resist **Poison**; Immune **Charmed, Exhaustion, Frightened, Paralyzed, Poisoned**. **Heated Body** (Metal): a creature that hits/grapples takes **1d10 Fire**. **Stony Lethargy** (Stone): one creature within 10 ft. makes **WIS save** or **Speed halved** and **no Opportunity Attacks** until its next turn. **Multiattack** = half slot level. **Slam**: **1d8 + 4 + slot** Bludgeoning. **Berserk Lashing** (Clay, reaction): on taking damage, **Slam** the source if possible, or move half Speed toward it without provoking.",
  scalingMd:
    "**Using a Higher-Level Spell Slot.** Use the **slot’s level** for the stat block’s level-based values.",
...
},
{
  ...
  ritual: false,
  descriptionMd:
    "You call forth an **elemental spirit** (choose **Air, Earth, Fire, or Water**) in an unoccupied space within range; it uses the **Elemental Spirit** stat block and vanishes at **0 HP** or when the spell ends. It acts after you, obeys **verbal commands** (no action), or **Dodges** if not commanded.\n\n**Stat Block (summary).** *Medium Elemental; AC **11 + slot level**; HP **50 + 10/slot above 4**; Speed **40 ft.**; **burrow 40 ft.** (Earth), **fly 40 ft. (hover)** (Air), **swim 40 ft.** (Water); Resists **Acid** (Water), **Lightning & Thunder** (Air), **Piercing & Slashing** (Earth); Immune **Fire** (Fire), **Poison**, and conditions **Exhaustion, Paralyzed, Petrified, Poisoned**. **Amorphous Form** (Air/Fire/Water): move through **1-inch** gaps. **Multiattack** = half slot level. **Slam**: **1d10 + 4 + slot** damage (type by element).",
  scalingMd:
    "**Using a Higher-Level Spell Slot.** Use the **slot’s level** for the stat block’s level-based values.",
